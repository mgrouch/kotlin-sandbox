...
object Landmarks {
    ...
    val Chimney = Symbol().apply {
        addInstr(SYMB, SubSymbol(Base, 1.0, 0.0, 0.0, null, null))
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        val p = Path2D.Double()
        p.moveTo(-25.0, 0.0)
        p.lineTo(-10.0, -120.0)
    ...
    val ChurchTower = Symbol().apply {
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(RECT, Rectangle2D.Double(-36.0, -36.0, 72.0, 72.0))
        addInstr(ELPS, Ellipse2D.Double(-2.0, -2.0, 4.0, 4.0))
    }

    val Cross = Symbol().apply {
        addInstr(SYMB, SubSymbol(Base, 1.0, 0.0, 0.0, null, null))
        addInstr(STRK, BasicStroke(6f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(LINE, Line2D.Double(0.0, -10.0, 0.0, -150.0))
        addInstr(LINE, Line2D.Double(-30.0, -115.0, 30.0, -115.0))
    }

    val DishAerial = Symbol().apply {
        addInstr(SYMB, SubSymbol(Base, 1.0, 0.0, 0.0, null, null))
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL))
        val p = Path2D.Double()
        p.moveTo(-7.8, -6.0)
        p.lineTo(0.0, -62.0)
        p.lineTo(7.8, -6.0)
    ...
    val Dome = Symbol().apply {
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(ELPS, Ellipse2D.Double(-36.0, -36.0, 72.0, 72.0))
    ...
    val LandTower = Symbol().apply {
        addInstr(SYMB, SubSymbol(Base, 1.0, 0.0, 0.0, null, null))
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(LINE, Line2D.Double(-25.0, 0.0, -15.0, -120.0))
        addInstr(LINE, Line2D.Double(25.0, 0.0, 15.0, -120.0))
        addInstr(RECT, Rectangle2D.Double(-15.0, -150.0, 30.0, 30.0))
    }
    ...
    val Monument = Symbol().apply {
        ...
        addInstr(LINE, Line2D.Double(-25.0, 0.0, -15.0, -105.0))
        addInstr(LINE, Line2D.Double(25.0, 0.0, 15.0, -105.0))
        addInstr(EARC, Arc2D.Double(-25.0, -150.0, 50.0, 50.0, 233.0, -285.0, Arc2D.OPEN))
    }

    val Platform = Symbol().apply {
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(RECT, Rectangle2D.Double(-48.0, -48.0, 96.0, 96.0))
        addInstr(RSHP, Ellipse2D.Double(-4.0, -4.0, 8.0, 8.0))
    }

    val RadioTV = Symbol().apply {
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_BEVEL))
        addInstr(EARC, Arc2D.Double(-30.0, -180.0, 60.0, 60.0, 45.0, -90.0, Arc2D.OPEN))
        addInstr(EARC, Arc2D.Double(-45.0, -195.0, 90.0, 90.0, 45.0, -90.0, Arc2D.OPEN))
        addInstr(EARC, Arc2D.Double(-30.0, -180.0, 60.0, 60.0, 225.0, -90.0, Arc2D.OPEN))
        addInstr(EARC, Arc2D.Double(-45.0, -195.0, 90.0, 90.0, 225.0, -90.0, Arc2D.OPEN))
    }
    ...
    val Minaret = Symbol().apply {
        addInstr(SYMB, SubSymbol(Spire, 1.0, 0.0, 0.0, null, null))
        addInstr(STRK, BasicStroke(6f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(LINE, Line2D.Double(0.0, -25.0, 0.0, -50.0))
        addInstr(STRK, BasicStroke(6f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER))
        addInstr(EARC, Arc2D.Double(-40.0, -110.0, 80.0, 60.0, 180.0, 180.0, Arc2D.OPEN))
    }

    val Temple = Symbol().apply {
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(RECT, Rectangle2D.Double(-25.0, -15.0, 50.0, 30.0))
        addInstr(STRK, BasicStroke(2f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(LINE, Line2D.Double(-35.0, -21.0, 35.0, 21.0))
        addInstr(LINE, Line2D.Double(-35.0, 21.0, 35.0, -21.0))
    }
    ...
    val WaterTower = Symbol().apply {
        ...
        addInstr(LINE, Line2D.Double(-25.0, 0.0, -15.0, -120.0))
        addInstr(LINE, Line2D.Double(25.0, 0.0, 15.0, -120.0))
        addInstr(RECT, Rectangle2D.Double(-25.0, -150.0, 50.0, 30.0))
    }

    val WindMotor = Symbol().apply {
        addInstr(SYMB, SubSymbol(Base, 1.0, 0.0, 0.0, null, null))
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER))
        addInstr(LINE, Line2D.Double(0.0, -10.0, 0.0, -90.0))
        addInstr(LINE, Line2D.Double(0.0, -90.0, 30.0, -90.0))
        addInstr(LINE, Line2D.Double(0.0, -90.0, -14.0, -116.6))
        addInstr(LINE, Line2D.Double(0.0, -90.0, -14.3, -66.7))
    }

    val Windmill = Symbol().apply {
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER))
        addInstr(ELPS, Ellipse2D.Double(-12.0, -12.0, 24.0, 24.0))
        addInstr(LINE, Line2D.Double(-30.0, -42.0, 30.0, 10.0))
        addInstr(LINE, Line2D.Double(-30.0, 10.0, 30.0, -42.0))
    }

    val Windsock = Symbol().apply {
        addInstr(SYMB, SubSymbol(Base, 1.0, 0.0, 0.0, null, null))
        addInstr(STRK, BasicStroke(4f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER))
        addInstr(LINE, Line2D.Double(0.0, -10.0, 0.0, -100.0))
        addInstr(STRK, BasicStroke(8f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER))
        addInstr(LINE, Line2D.Double(0.0, -100.0, 0.0, -150.0))
